...
- **来場者との紐づけ**: 質問の中で氏名や連絡先を尋ねることはしませんが、開始画面で来場者の QR コードを読み取ると、その文字列が突合用の外部ID（`sessions.external_id`）として回答に紐づけて記録されます（読み取りは任意）。回答者に「匿名」と案内しない前提の設計です
...
開始画面の QR コード読み取りはカメラを使うため、セキュアコンテキスト（`localhost` または HTTPS）でのみ動作します。スマートフォン実機で確認する場合は、LAN の `http://` アドレスではカメラが使えないので、`cloudflared tunnel --url http://localhost:3001` などで HTTPS の一時 URL を作るか、Workers にデプロイして確認してください。
